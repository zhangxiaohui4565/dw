...
insert overwrite table ${APP}.dwd_dim_sku_info partition(dt='$dt')
select
    sku.id,
    sku.spu_id,
    sku.price,
...
from
(
    select * from ${APP}.ods_sku_info where dt='$dt'
)sku
join
(
    select * from ${APP}.ods_base_trademark where dt='$dt'
)ob on sku.tm_id=ob.tm_id
join
(
    select * from ${APP}.ods_spu_info where dt='$dt'
)spu on spu.id = sku.spu_id
join
(
    select * from ${APP}.ods_base_category3 where dt='$dt'
)c3 on sku.category3_id=c3.id
join
(
    select * from ${APP}.ods_base_category2 where dt='$dt'
)c2 on c3.category2_id=c2.id
join
(
    select * from ${APP}.ods_base_category1 where dt='$dt'
)c1 on c2.category1_id=c1.id;


insert overwrite table ${APP}.dwd_dim_coupon_info partition(dt='$dt')
select
    id,
    coupon_name,
    coupon_type,
...
insert overwrite table ${APP}.dwd_dim_activity_info partition(dt='$dt')
select
    id,
    activity_name,
    activity_type,
...
insert overwrite table ${APP}.dwd_fact_order_detail partition(dt='$dt')
select
    id,
    order_id,
    user_id,
...
(
    ...
    from
    (
        select * from ${APP}.ods_order_detail where dt='$dt'
    ) od
    join
    (
        select * from ${APP}.ods_order_info where dt='$dt'
    ) oi
    on od.order_id=oi.id
)t1;

insert overwrite table ${APP}.dwd_fact_payment_info partition(dt='$dt')
select
    pi.id,
    pi.out_trade_no,
    pi.order_id,
...
from
(
    select * from ${APP}.ods_payment_info where dt='$dt'
)pi
join
(
    select id, province_id from ${APP}.ods_order_info where dt='$dt'
)oi
on pi.order_id = oi.id;


insert overwrite table ${APP}.dwd_fact_order_refund_info partition(dt='$dt')
select
    id,
    user_id,
    order_id,
...
insert overwrite table ${APP}.dwd_fact_comment_info partition(dt='$dt')
select
    id,
    user_id,
    sku_id,
...
insert overwrite table ${APP}.dwd_fact_cart_info partition(dt='$dt')
select
    id,
    user_id,
    sku_id,
...
insert overwrite table ${APP}.dwd_fact_favor_info partition(dt='$dt')
select
    id,
    user_id,
    sku_id,
...
(
    ...
    left join
    (
        select * from ${APP}.ods_activity_order where dt='$dt'
    )act
...
(
    select
        ...
        '$dt' start_date,
        '9999-99-99' end_date
    from ${APP}.ods_user_info where dt='$dt'
